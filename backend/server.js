...
app.post("/api/public/contact", (req, res) => {
  ...
  if (!name || !email || !message) {
    return res.status(400).send(`<!DOCTYPE html><html><head><title>Contact Error</title><style>body{font-family:system-ui,sans-serif;max-width:600px;margin:2rem auto;padding:0 1rem}pre{background:#f6f6f6;padding:.75rem;border-radius:4px}.error{color:#c0392b;font-weight:600}a{display:inline-block;margin-top:1rem}</style></head><body><h1>Contact Form</h1><p class="error">Error: name, email, and message are all required.</p><a href="javascript:history.back()">Go back</a></body></html>`);
  }

  if (typeof name !== "string" || typeof email !== "string" || typeof message !== "string") {
    return res.status(400).send(`<!DOCTYPE html><html><head><title>Contact Error</title><style>body{font-family:system-ui,sans-serif;max-width:600px;margin:2rem auto;padding:0 1rem}.error{color:#c0392b;font-weight:600}a{display:inline-block;margin-top:1rem}</style></head><body><h1>Contact Form</h1><p class="error">Error: name, email, and message must be strings.</p><a href="javascript:history.back()">Go back</a></body></html>`);
  }

  if (name.trim().length === 0 || email.trim().length === 0 || message.trim().length === 0) {
    return res.status(400).send(`<!DOCTYPE html><html><head><title>Contact Error</title><style>body{font-family:system-ui,sans-serif;max-width:600px;margin:2rem auto;padding:0 1rem}.error{color:#c0392b;font-weight:600}a{display:inline-block;margin-top:1rem}</style></head><body><h1>Contact Form</h1><p class="error">Error: name, email, and message must not be empty.</p><a href="javascript:history.back()">Go back</a></body></html>`);
  }
...
<style>body{font-family:system-ui,sans-serif;max-width:600px;margin:2rem auto;padding:0 1rem}pre{background:#f6f6f6;padding:.75rem;border-radius:4px;overflow-x:auto}.success{color:#2c7a39;font-weight:600}a{display:inline-block;margin-top:1rem}</style>
</head>
<body>
